...
class PaymentMethod
{
    ...
    public function add()
    {
        ...
        if ($this->id > 0)
        {
            $sql = "UPDATE payment_methods SET `name`= ?, `status`= ?, `notes`= ? "
                 . " WHERE id = ?  AND payment_methods.client_id = ? ";
            ...
            $maxId = $this->db->getMaxId("id", "payment_methods");
            $sql = "INSERT INTO payment_methods (`id`, `client_id`, `name`, `status`, `notes`) "
                 . "VALUES(?,?,?,?,?)";
            ...
            $bind = $stmt->bind_param("iisis", $maxId, $this->clientId, $this->name, 
    ...
    public static function isNameExist($p_name, $p_id=0)
    {
        ...
        $sql = "SELECT payment_methods.`id` FROM payment_methods  WHERE "
             . " name = '"  . $p_name . "' AND payment_methods.client_id = " . CLIENT_ID;
    ...
    public static function getAll(array $searchData=array(), $startIndex=0, $limit=DATA_PER_PAGE)
    {
        ...
        $sql = "SELECT `id`, `client_id`, `name`, "
             . " `status`, `notes`, `added_date` AS addedDate FROM payment_methods "
             . " WHERE payment_methods.`client_id` = " . CLIENT_ID;
        ...
        $sql .= " ORDER BY payment_methods.added_date DESC  LIMIT " . $limit ." OFFSET " . $startIndex;
    ...
    public function getDetails($pId)
    {
        ...
        $sql = "SELECT `id`, `client_id`, `name`, "
             ...
             . " FROM `payment_methods` WHERE id = " . $pId . " AND client_id = " . CLIENT_ID;
    ...
    public static function delete($ids)
    {
        ...
        $sql = "DELETE FROM payment_methods WHERE id IN (" . $ids . ")  AND client_id = " . CLIENT_ID;
    ...
    public static function getCount($pStatus=0, $onlyNew=false)
    {
        ...
        $sql = "SELECT id FROM payment_methods WHERE client_id = " . CLIENT_ID;
    ...
    public static function getNames($status=ACTIVE)
    {
        ...
        $sql = "SELECT payment_methods.id, payment_methods.name FROM payment_methods "
             . " WHERE payment_methods.client_id = " . CLIENT_ID;
        
        
        if ((int)$status === ACTIVE || (int)$status === INACTIVE){
            $sql .= " AND payment_methods.status='" . $status . "' ";
        }
