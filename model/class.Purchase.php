...
class Purchase
{
    ...
    public function add()
    {
        ...
        if ($this->id > 0){
            $sql = "UPDATE purchases SET `supplier_id` = ?,`product_id` = ?,`quantity` = ?, `measuring_unit_id`= ?,`unit_price` = ?, "
                 . " `amount`= ?, `invoice_number`= ?,`purchase_order_no`= ?,`payment_term_duration`= ?,  "
            ...
            $bind = $stmt->bind_param("iiiiddssiisiisii",  $this->supplierId, $this->productId, $this->quantity,
                                       $this->measuringUnitId, $this->unitPrice, $this->amount, $this->invoiceNumber, 
            ...
            $sql = "INSERT INTO purchases (`id`, `client_id`, `supplier_id`, `product_id`, `quantity`, "
                 . " `measuring_unit_id`, `unit_price`, `amount`, `invoice_number`, `purchase_order_no`, "
            ...
            $bind = $stmt->bind_param("iiiiiddissiisiis", $maxId, $this->clientId, $this->supplierId, $this->productId, 
                                      $this->quantity, $this->measuringUnitId, $this->unitPrice, $this->amount, $this->invoiceNumber,
    ...
    public function getAll(array $searchData=array(), $setTotalAmount=true, $startIndex=0, $limit=DATA_PER_PAGE)
    {
        ...
        $sql = "SELECT purchases.`id`, purchases.`client_id` AS clientId, purchases.`supplier_id` AS supplierId, "
             ...
             . " purchases.`amount`, purchases.`measuring_unit_id` AS measuringUnitId, purchases.`invoice_number` AS invoiceNumber, "
             ...
             . " LEFT JOIN measuring_units ON purchases.measuring_unit_id = measuring_units.id AND measuring_units.client_id = " . CLIENT_ID
        ...
        if (isset($searchData['productId']) && trim($searchData['productId']) > 0){
            $searchSql .= " purchases.product_id = '" . trim($searchData['productId']) . "' OR ";
        }
        
        if (isset($searchData['measuringUnitId']) && trim($searchData['measuringUnitId']) > 0){
            $searchSql .= " purchases.measuring_unit_id = '" . trim($searchData['measuringUnitId']) . "' OR ";
        }
