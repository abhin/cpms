...
class EmploymentType
{
    ...
    public function add()
    {
        ...
        if ($this->id > 0)
        {
            $sql = "UPDATE employment_types SET `name`= ?, `status`= ?, `notes`= ? "
                 . " WHERE id = ?  AND employment_types.client_id = ? ";
            ...
            $maxId = $this->db->getMaxId("id", "employment_types");
            $this->set('addedDate', date('Y-m-d H:i:s', strtotime("now")));
            $sql = "INSERT INTO employment_types (`id`, `client_id`, `name`, `status`, `notes`, `added_date`) "
                 . "VALUES(?,?,?,?,?,?)";
    ...
    public static function isNameExist($p_name, $p_id=0)
    {
        ...
        $sql = "SELECT employment_types.`id` FROM employment_types  WHERE "
             . " name = '"  . $p_name . "' AND employment_types.client_id = " . CLIENT_ID;
    ...
    public static function getAll(array $searchData=array(), $startIndex=0, $limit=DATA_PER_PAGE)
    {
        ...
        $sql = "SELECT `id`, `client_id` AS clientId, `name`, "
             . " `status`, `notes`, `added_date` AS addedDate FROM employment_types "
             . " WHERE employment_types.`client_id` = " . CLIENT_ID;
        ...
        $sql .= " ORDER BY employment_types.added_date DESC  LIMIT " . $limit ." OFFSET " . $startIndex;
    ...
    public function getDetails($pId)
    {
        ...
        $sql = "SELECT `id`, `client_id` AS clientId, `name`, "
             ...
             . " FROM `employment_types` WHERE id = " . $pId . " AND client_id = " . CLIENT_ID;
    ...
    public static function delete($ids)
    {
        ...
        $sql = "DELETE FROM employment_types WHERE id IN (" . $ids . ")  AND client_id = " . CLIENT_ID;
    ...
    public static function getCount($pStatus=0, $onlyNew=false)
    {
        ...
        $sql = "SELECT id FROM employment_types WHERE client_id = " . CLIENT_ID;
    ...
    public static function getNames($status=ACTIVE)
    {
        ...
        $sql = "SELECT employment_types.id, employment_types.name FROM employment_types "
             . " WHERE employment_types.client_id = " . CLIENT_ID;
        
        if ((int)$status === ACTIVE || (int)$status === INACTIVE){
            $sql .= " AND employment_types.status='" . $status . "' ";
        }
        
        $sql .= " ORDER BY employment_types.name ASC";
