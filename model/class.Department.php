...
class Department
{
    ...
    public function add()
    {
        ...
        if ($this->id > 0)
        {
            $sql = "UPDATE departments SET `name`= ?, `status`= ?, `notes`= ? "
                 . " WHERE id = ?  AND departments.client_id = ? ";
            ...
            $maxId = $this->db->getMaxId("id", "departments");
            $this->set('addedDate', date('Y-m-d H:i:s', strtotime("now")));
            $sql = "INSERT INTO departments (`id`, `client_id`, `name`, `status`, `notes`, `added_date`) "
                 . "VALUES(?,?,?,?,?,?)";
    ...
    public static function isNameExist($p_name, $p_id=0)
    {
        ...
        $sql = "SELECT departments.`id` FROM departments  WHERE "
             . " name = '"  . $p_name . "' AND departments.client_id = " . CLIENT_ID;
    ...
    public static function getAll(array $searchData=array(), $startIndex=0, $limit=DATA_PER_PAGE)
    {
        ...
        $sql = "SELECT `id`, `client_id` AS clientId, `name`, "
             . " `status`, `notes`, `added_date` AS addedDate FROM departments "
        ...
        $sql .= " ORDER BY departments.added_date DESC  LIMIT " . $limit ." OFFSET " . $startIndex;
    ...
    public function getDetails($pId)
    {
        ...
        $sql = "SELECT `id`, `client_id` AS clientId, `name`, "
             ...
             . " FROM `departments` WHERE id = " . $pId . " AND client_id = " . CLIENT_ID;
    ...
    public static function delete($ids)
    {
        ...
        $sql = "DELETE FROM departments WHERE id IN (" . $ids . ")  AND client_id = " . CLIENT_ID;
    ...
    public static function getCount($pStatus=0, $onlyNew=false)
    {
        ...
        $sql = "SELECT id FROM departments WHERE client_id = " . CLIENT_ID;
    ...
    public static function getNames($status=0)
    {
        ...
        if ((int)$status === 1 || (int)$status === 2){
            $sql .= " AND departments.status='" . $status . "' ";
        }
        
        $sql .= " ORDER BY departments.name ASC";
