...
class EducationCourse
{
    ...
    public function add()
    {
        ...
        if ($this->id > 0)
        {
            $sql = "UPDATE education_courses SET `name`= ?, `status`= ?, `notes`= ? "
                 . " WHERE id = ?  AND education_courses.client_id = ? ";
            ...
            $maxId = $this->db->getMaxId("id", "education_courses");
            $this->set('addedDate', date('Y-m-d H:i:s', strtotime("now")));
            $sql = "INSERT INTO education_courses (`id`, `client_id`, `name`, `status`, `notes`, `added_date`) "
                 . "VALUES(?,?,?,?,?,?)";
    ...
    public static function isNameExist($p_name, $p_id=0)
    {
        ...
        $sql = "SELECT education_courses.`id` FROM education_courses  WHERE "
             . " name = '"  . $p_name . "' AND education_courses.client_id = " . CLIENT_ID;
    ...
    public static function getAll(array $searchData=array(), $startIndex=0, $limit=DATA_PER_PAGE)
    {
        ...
        $sql = "SELECT `id`, `client_id` AS clientId, `name`, "
             . " `status`, `notes`, `added_date` AS addedDate FROM education_courses "
             . " WHERE education_courses.`client_id` = " . CLIENT_ID;
        ...
        $sql .= " ORDER BY education_courses.added_date DESC  LIMIT " . $limit ." OFFSET " . $startIndex;
    ...
    public function getDetails($pId)
    {
        ...
        $sql = "SELECT `id`, `client_id` AS clientId, `name`, "
             ...
             . " FROM `education_courses` WHERE id = " . $pId . " AND client_id = " . CLIENT_ID;
    ...
    public static function delete($ids)
    {
        ...
        $sql = "DELETE FROM education_courses WHERE id IN (" . $ids . ")  AND client_id = " . CLIENT_ID;
    ...
    public static function getCount($pStatus=0, $onlyNew=false)
    {
        ...
        $sql = "SELECT id FROM education_courses WHERE client_id = " . CLIENT_ID;
    ...
    public static function getNames()
    {
        ...
        $sql = "SELECT education_courses.id, education_courses.name FROM education_courses "
             . " WHERE education_courses.client_id = " . CLIENT_ID . " AND education_courses.status='" . ACTIVE . "' ORDER BY name ASC";
    ...
    //----------------------------------------------------------------------------------------------
    public static function getCourseNames($ids)
    {
        $oMysqli = $GLOBALS[DEFAULT_DBO_NAME]->connectToDb();
        $names = array();
        
        $sql = "SELECT education_courses.name FROM education_courses "
             . " WHERE education_courses.client_id = " . CLIENT_ID . " AND id IN(" . $ids . ")";
