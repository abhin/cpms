...
if(isset($_POST['name']) && isset($_GET[md5('do')]) && $_GET[md5('do')] == VALIDATE) 
{
  ...
  $status = PaymentMethod::isNameExist($name, $userId);
  
  if($status) {
    // PaymentMethod name is registered on another account
    $response = array('valid' => false, 'message' => 'This payment method already exist.');
  } else {
    // PaymentMethod name is available
...
else if(isset($_POST['loadDataAjax']) && isset($_POST['start'])) 
{
  ...
  $i = $startIndex+1;
  $data = $oPaymentMethod->getAll($searchData, $startIndex);
