...
if(isset($_POST['name']) && isset($_GET[md5('do')]) && $_GET[md5('do')] == VALIDATE) 
{
  ...
  $status = Designation::isNameExist($name, $userId);
  
  if($status) {
    // Designation name is registered on another account
    $response = array('valid' => false, 'message' => 'This payment term already exist.');
  } else {
    // Designation name is available
...
else if(isset($_POST['loadDataAjax']) && isset($_POST['start'])) 
{
  ...
  $i = $startIndex+1;
  $data = $oDesignation->getAll($searchData, $startIndex);
