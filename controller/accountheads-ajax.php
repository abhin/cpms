...
if(isset($_POST['name']) && isset($_GET[md5('do')]) && $_GET[md5('do')] == VALIDATE) 
{
  ...
  $status = AccountHead::isNameExist($name, $userId);
  
  if($status) {
    // AccountHead name is registered on another account
    $response = array('valid' => false, 'message' => 'This account head already exist.');
  } else {
    // AccountHead name is available
...
else if(isset($_POST['loadDataAjax']) && isset($_POST['start'])) 
{
  ...
  $i = $startIndex+1;
  $data = $oAccountHead->getAll($searchData, $startIndex);
    ...
    foreach ($data as $index=>$details)
    {
        ...
        $tableRow[$i] .= $details->notes .", "
                    . '<a class="btn btn-info btn-small" data-toggle="tooltip" data-original-title="Edit project." href="' . Security::actionUrl($actionPage, array("id"=>$details->id, 'do'=>EDIT)) . '">
                                                   <i class="glyphicon glyphicon-edit icon-white"></i>
                                                   Edit
                                               </a>
                                               <a class="btn btn-danger btn-small delete" data-toggle="tooltip" data-original-title="Delete project." href="' . Security::actionUrl($actionPage, array("id"=>$details->id, 'do'=>DELETE)) . '">
