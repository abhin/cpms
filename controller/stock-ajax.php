...
//  $i =  ($startIndex / ($DATA_PER_PAGE / PriceType::getCount()));
  ...
  $data = $oStock->getAll(array(), $startIndex);
  ...
  foreach ($data as $index=>$details)
  {
       ...
       $tableRow[$i] .= $details['notes'] .", "
                      . '<a class="btn btn-info btn-small" href="' . Security::actionUrl($actionPage, array("id"=>$details['productId'], "do"=>1)) . '">
                               <i class="glyphicon glyphicon-edit icon-white"></i>
                               Edit
                           </a>
                           <a class="btn btn-danger btn-small" href="' . Security::actionUrl($actionPage, array("id"=>$details['productId'], "do"=>2)) . '">
