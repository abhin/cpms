...
{block  name="title" prepend}Account Heads{/block}
            ...
            <div class="box col-md-12">
                <div class="box-inner">
                    <div class="box-header well" onclick="return false;">
                        <h2><i class="glyphicon glyphicon-plus-sign"></i>&nbsp;{if isset($a_TemplateData['accountHeadData']['id']) && $a_TemplateData['accountHeadData']['id'] > 0}Edit{else}Add New{/if}</h2>
                    ...
                    {if isset($a_TemplateData['accountHeadData']['showForm'])}
                    ...
                    <div class="box-content row" style="{if $showForm}display: block;{else}display: none;{/if}">
                        <div class="col-lg-12 col-md-12">
                                {if isset($a_TemplateData['accountHeadData']['id']) &&  $a_TemplateData['accountHeadData']['id'] > 0}
                                    {assign var=accountheadsId value={$a_TemplateData['accountHeadData']['id']}}
                                {else}
                                    {assign var=accountheadsId value=0}
                                {/if}
                                <input type="hidden" name="id" value="{$accountheadsId}"/>
                                    
                                <div class="form-group col-xs-4">
                                    <label for="name">
                                        Name
                                        <a data-original-title="Name of account head" data-toggle="tooltip" title="">
                                            <i class="glyphicon glyphicon-question-sign"></i>
                                        </a>
                                    </label>
                                    <input type="text" id="name" name="name" class="form-control" data-validation="alphanumeric server" data-validation-allowing="{$a_TemplateData['formValidChars']}" data-validation-url="{actionurl page=$ajaxFilePath params=["do"=>$a_TemplateData['VALIDATE']]}" data-validation-error-msg="Alphanumeric values only" value='{if isset($a_TemplateData['accountHeadData']['name'])}{$a_TemplateData['accountHeadData']['name']}{/if}' autocomplete="off" placeholder="Account Head name"/>
                                </div>
                                
                                <div class="form-group col-xs-4">
                                    {if isset ($a_TemplateData['accountHeadData']['accountTypeId'])}
                                        {assign var="accountTypeId" value=$a_TemplateData['accountHeadData']['accountTypeId']}
                                    {else}
                                        {assign var="accountTypeId" value=0}
                                    {/if}
                                    <label for="accountTypeId">Account Types</label>
                                    <select id="accountTypeId" name="accountTypeId" class="form-control chosen-select" data-validation="number" data-validation-error-msg="Select type" data-placeholder="Choose...">
                                        <option value=""></option>
                                        {foreach $a_TemplateData['allAccountType'] as $details}
                                            <option value="{$details->id}" {if $accountTypeId == $details->id}selected='selected'{/if}>
                                                {$details->name}
                                            </option>
                                        {/foreach}
                                    </select>
                                ...
                                <div class="form-group col-xs-4">
                                    {if isset ($a_TemplateData['accountHeadData']['status'])}
                                        {assign var="status" value=$a_TemplateData['accountHeadData']['status']}
                                    {else}
                                ...
                                <div class="form-group col-xs-4">
                                    ...
                                    <textarea id="notes" name="notes" class="form-control" data-validation="alphanumeric" data-validation-allowing="{$a_TemplateData['formValidChars']}" data-validation-error-msg="Alphanumeric value only" data-validation-optional="true"> {if isset($a_TemplateData['accountHeadData']['notes'])}{$a_TemplateData['accountHeadData']['notes']}{/if}</textarea>
                                </div>
                                </div>
                                
                                <div class="form-group col-xs-12" style="border:0px solid red;text-align: center; padding-top: 35px;">
                                    <input class="btn btn-primary" id='addAccountHead'  type="submit" name="add_accountHead" value="{if isset($a_TemplateData['accountHeadData']['id']) && $a_TemplateData['accountHeadData']['id'] > 0}Save{else}Add{/if}"/>&nbsp;&nbsp;&nbsp;
        ...
        <div class="row">
            <div class="box col-md-12">
                <div class="box-inner">
                    ...
                    {*Search Form*}
                    {if isset($a_TemplateData['searchData']['showForm'])}
                        {assign var=showForm value=true}
                    {else}
                        {assign var=showForm value=false}
                    {/if}
                    ...
                    <div class="box-content row" style="{if $showForm}display: block;{else}display: none;{/if}">
                        <div class="col-lg-7 col-md-12 formContainer">
                                <div class="form-group col-xs-4">
                                    ...
                                    <input type="text" name="name" class="form-control" data-validation="alphanumeric" data-validation-allowing="{$a_TemplateData['formValidChars']}"  data-validation-error-msg="Alphanumeric values only"  data-validation-optional="true" value='{if isset($a_TemplateData['searchData']['name'])}{$a_TemplateData['searchData']['name']}{/if}' autocomplete="off" placeholder="Account Head name"/>
                                </div>
                                
                                <div class="form-group col-xs-4">
                                    {if isset ($a_TemplateData['searchData']['accountTypeId'])}
                                        {assign var="accountTypeId" value=$a_TemplateData['searchData']['accountTypeId']}
                                    {else}
                                        {assign var="accountTypeId" value=0}
                                    {/if}
                                    <label for="accountTypeId">Account Types</label>
                                    <select name="accountTypeId" class="form-control chosen-select" data-validation="number" data-validation-error-msg="Select account type" data-validation-optional="true" data-placeholder="Choose...">
                                        <option value=""></option>
                                        {foreach $a_TemplateData['allAccountType'] as $details}
                                            <option value="{$details->id}" {if $accountTypeId == $details->id}selected='selected'{/if}>
                                                {$details->name}
                                ...
                                <div class="form-group col-xs-12" style="border:0px solid red;text-align: center; padding-top: 35px;">
                                    <input class="btn btn-primary" type="submit" name="search_accountHead" value="Search"/>&nbsp;&nbsp;&nbsp;
                            ...
                            <table id="tableData" class="display" cellspacing="0" width="100%" data-order='[[ 1, "asc" ]]'>
                                ...
                                <tfoot>
                                  <tr class="tablesorter-headerRow">
                                    ...
                                    <tbody>
                                    {if $a_TemplateData['allAccountHeads']}
                                        {foreach $a_TemplateData['allAccountHeads'] as $index=>$details}    
                                            <tr class="{if $index % 2 == 0}odd{else}even{/if}">
                                               ...
                                               <td>{$details->notes}</td>
                                               <td>
                                                   <a class="btn btn-info btn-small" data-toggle="tooltip" data-original-title="Edit account type." href="{actionurl page=$actionPage params=['id'=>$details->id, 'do'=>$a_TemplateData["EDIT"]]}">
                                                       <i class="glyphicon glyphicon-edit icon-white"></i>
                                                       Edit
                                                   </a>
                                                   <a class="btn btn-danger btn-small" data-toggle="tooltip" data-original-title="Delete account type." href="{actionurl page=$actionPage params=['id'=>$details->id, 'do'=>$a_TemplateData['DELETE']]}">
...
{block name="jsScriptBottoom"}
    ...
    <script>
        function init(){
            ...
            var tableOptions = {};
            {/literal}
            {if isset($a_TemplateData['allAccountHeads']) && $a_TemplateData['allAccountHeads']}
            {literal}
