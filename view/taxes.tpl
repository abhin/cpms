...
{block  name="title" prepend}Taxes{/block}
            ...
            <div class="box col-md-12">
                <div class="box-inner">
                    <div class="box-header well" onclick="return false;">
                        <h2><i class="glyphicon glyphicon-plus-sign"></i>&nbsp;{if isset($a_TemplateData['taxData']['id']) && $a_TemplateData['taxData']['id'] > 0}Edit{else}Add New{/if}</h2>
                    ...
                    {if isset($a_TemplateData['taxData']['showForm'])}
                    ...
                    <div class="box-content row" style="{if $showForm}display: block;{else}display: none;{/if}">
                        <div class="col-lg-12 col-md-12">
                            <form action="{actionurl page=$actionPage}" method="post" class="form-inline addForm"  style="text-align: center;">
                                {if isset($a_TemplateData['taxData']['id']) &&  $a_TemplateData['taxData']['id'] > 0}
                                    {assign var=taxId value={$a_TemplateData['taxData']['id']}}
                                {else}
                                ...
                                <div class="form-group col-xs-4">
                                    ...
                                    <input type="text" id="name" name="name" class="form-control" data-validation="alphanumeric server" data-validation-allowing="{$a_TemplateData['formValidChars']}"  data-validation-url="{actionurl page=$ajaxFilePath params=["do"=>$a_TemplateData['VALIDATE'], "id"=>$taxId]}" data-validation-error-msg="Alphanumeric values only" value='{if isset($a_TemplateData['taxData']['name'])}{$a_TemplateData['taxData']['name']}{/if}' autocomplete="off" placeholder="Tax name"/>
                                </div>
                                ...
                                <div class="form-group col-xs-4">
                                    ...
                                    <div class="input-group" style="width:100% !important;">
                                        <input type="text" id="precentage" name="precentage" class="form-control" data-validation="number" data-validation-error-msg="Invalid precentage"  data-validation-allowing="float" value='{if isset($a_TemplateData['taxData']['precentage'])}{$a_TemplateData['taxData']['precentage']}{/if}' autocomplete="off" placeholder="Tax Precentage" style="width:100% !important;"/>
                                        <div class="input-group-addon">%</div>
                                    </div>
                                </div>
                                
                                <div class="form-group col-xs-4">
                                    {if isset ($a_TemplateData['taxData']['isDefault'])}
                                        {assign var="isDefault" value=$a_TemplateData['taxData']['isDefault']}
                                    {else}
                                        {assign var="isDefault" value=2}
                                    {/if}
                                    <label for="is" >
                                        Is Default
                                    </label>
                                    <select id="isDefault" name="isDefault" class="form-control chosen-select" data-validation="number" data-validation-error-msg="Invalid selection" >
                                        <option value="1" {if $isDefault == 1}selected='selected'{/if}>
                                            Yes
                                        </option>
                                        <option value="2" {if $isDefault == 2}selected='selected'{/if}>
                                            No
                                        </option>
                                    </select>
                                </div>
                                ...
                                <div class="col-xs-12">
                                <div class="form-group col-xs-4">
                                    {if isset ($a_TemplateData['taxData']['status'])}
                                        {assign var="status" value=$a_TemplateData['taxData']['status']}
                                    {else}
                                ...
                                <div class="form-group col-xs-4">
                                    ...
                                    <textarea id="notes" name="notes" class="form-control" data-validation="alphanumeric" data-validation-allowing="{$a_TemplateData['formValidChars']}" data-validation-error-msg="Alphanumeric value only" data-validation-optional="true"> {if isset($a_TemplateData['taxData']['notes'])}{$a_TemplateData['taxData']['notes']}{/if}</textarea>
                                </div>
                                </div>
                                
                                <div class="form-group col-xs-12" style="border:0px solid red;text-align: center; padding-top: 35px;">
                                    <input class="btn btn-primary" id='addTax'  type="submit" name="add_tax" value="{if isset($a_TemplateData['taxData']['id']) && $a_TemplateData['taxData']['id'] > 0}Save{else}Add{/if}"/>&nbsp;&nbsp;&nbsp;
        ...
        <div class="row">
            <div class="box col-md-12">
                <div class="box-inner">
                    ...
                    {*Search Form*}
                    {if isset($a_TemplateData['searchData']['search_tax'])}
                        {assign var=showForm value=true}
                    {else}
                        {assign var=showForm value=false}
                    {/if}
                        ...
                        <div class="col-lg-7 col-md-12 formContainer">
                            <form action="{actionurl page=$actionPage}" method="post" class="form-inline searchForm"  style="text-align: center;">
                                ...
                                <div class="form-group col-xs-4">
                                    ...
                                    <div class="input-group" style="width:100% !important;">
                                        <input type="text" name="precentage" class="form-control" data-validation="number" data-validation-error-msg="Invalid precentage"  data-validation-allowing="float" data-validation-optional="true" value='{if isset($a_TemplateData['searchData']['precentage'])}{$a_TemplateData['searchData']['precentage']}{/if}' autocomplete="off" placeholder="Tax Precentage" style="width:100% !important;"/>
                                        <div class="input-group-addon">%</div>
                                    </div>
                                </div>
                ...
                <div class="box-inner">
                    ...
                    <div class="box-content">
                            ...
                            <table id="tableData" class="display" cellspacing="0" width="100%" data-order='[[ 1, "asc" ]]'>
                                ...
                                <tbody>
                                    {if $a_TemplateData['allTaxes']}
                                        {foreach $a_TemplateData['allTaxes'] as $index=>$details}    
                                            <tr class="{if $index % 2 == 0}odd{else}even{/if}">
                                               ...
                                               <td>{$index + 1}</td>
                                               <td>{$details->name}</td>
                                               <td>{$details->precentage}%</td>
                                               <td>
                                                    {if $details->isDefault != 1}
                                                        <input type="radio" name="notDefaultTax" value="{$details->id}" />
                                                    {else}
                                                        <span id="defaultTax_{$details->id}">
                                               ...
                                               <td >{$details->notes}</td>
                                               <td>
                                                   <a class="btn btn-info btn-small" data-toggle="tooltip" data-original-title="Edit tax." href="{actionurl page=$actionPage params=['id'=>$details->id, 'do'=>$a_TemplateData['EDIT']]}">
                                                       <i class="glyphicon glyphicon-edit icon-white"></i>
                                                       Edit
                                                   </a>
                                                   <a class="btn btn-danger btn-small delete" data-toggle="tooltip" data-original-title="Delete tax." href="{actionurl page=$actionPage params=['id'=>$details->id, 'do'=>$a_TemplateData['DELETE']]}">
...
{block name="jsScriptBottoom"}
    ...
    <script>
        function init(){
            ...
            var tableOptions = {};
            {/literal}
            {if isset($a_TemplateData['allTaxes']) && $a_TemplateData['allTaxes']}
            {literal}
                ...
                loadDataTable('#tableData', '{/literal}{actionurl page=$ajaxFilePath}{literal}', {/literal}{$a_TemplateData['DATA_PER_PAGE']}{literal}, tableOptions, setDefaultCallback);
            
            function setDefaultCallback(){
                setDefaultTax('{/literal}{actionurl page=$ajaxFilePath}{literal}');
            }
            setDefaultCallback();
        }
    </script>
